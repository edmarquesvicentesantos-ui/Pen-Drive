...
function imprimirRecibo(cliente, itens, total, tipo) {
    ...
    let lista = itens.map(i => `<tr><td>${i.qtd}x ${i.nome}</td><td align="right">${i.subtotal.toFixed(2)}</td></tr>`).join('');
    win.document.write(`<html><body onload="window.print();window.close()"><div style="font-family:monospace;width:100%"><center><strong>BOTECO 934</strong><br>--------------------</center><table style="width:100%">${lista}</table>--------------------<br><strong>TOTAL: R$ ${total.toFixed(2)}</strong><br>PAGTO: ${tipo}</div></body></html>`);
    win.document.close();
}
...
document.getElementById('biparVenda').addEventListener('change', async (e) => {
    ...
    if (!snap.empty) {
        ...
        else { carrinho.push({ nome: p.nome, preco: p.precoVenda, codigo: code, qtd: 1, subtotal: p.precoVenda, idBanco: d.id }); }
        atualizarTela();
    }
...
function atualizarTela() {
    ...
    corpo.innerHTML = carrinho.map(i => `<tr><td>${i.qtd}x</td><td>${i.nome}</td><td>${i.preco.toFixed(2)}</td><td>${i.subtotal.toFixed(2)}</td></tr>`).join('');
...
window.finalizarVenda = async () => {
    ...
    if (confirm("Deseja Imprimir Cupom?")) imprimirRecibo(cliente, carrinho, total, pag);
    
    carrinho = []; atualizarTela(); document.getElementById('identificacaoCliente').value = "";
};
...
onSnapshot(collection(db, "estoque"), (s) => {
    ...
    s.forEach(d => {
        ...
        g.innerHTML += `<div><small>${p.nome}</small><div class="barra-fora"><div class="barra-dentro" style="width:${porc}%; background:${porc < 20 ? '#ff5e5e' : '#1fcc7d'}"></div></div></div>`;
    });
});
...
document.getElementById('arquivoImportar').addEventListener('change', function(e) {
    ...
    reader.onload = (ev) => {
        ...
        document.getElementById('corpoConferencia').innerHTML = produtosTemporarios.map(p => `<p>${p.nome} - R$ ${p.precoVenda}</p>`).join('');
